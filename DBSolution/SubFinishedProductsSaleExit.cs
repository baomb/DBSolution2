...
{
    public partial class SubFinishedProductsSaleExit : Form
    {
        ...
        public SubFinishedProductsSaleExit()
        {
            InitializeComponent();
            InitForm();
            if (!readPort)
        ...
        private void toolStripButtonSave_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("确实要保存吗?", "史丹利地磅系统-子公司成品销售出厂", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                ...
                {
                    dtUpData.Rows[i]["WERKS"] = strWerks;
                ...
                try
                {
                    ...
                    try
                    {
                        dtGZ = SAPHelper.InvokSAPFunTable("Z_SDL_FERT_MUTUAL_ZGS_GZ", ltPara, ltParaTb, lt, ref lr);
                    }
                    catch
                    {
                        ...
                        try
                        {
                            ...
                            for (int i = 0; i < dataGridViewSelect.Rows.Count; i++)
                            {
                                if (dataGridViewSelect.Rows[i].Cells[0].EditedFormattedValue.ToString().ToUpper() == "TRUE")
                                {
                                    ...
                                    dr["MZ"] = textBoxGross.Text.ToString();
                                    dr["NUMBR"] = textBoxCar.Text.ToString();
                                    dr["FLAG"] = "1";
                                    dtNewWW.Rows.Add(dr);
                                }
                            }
