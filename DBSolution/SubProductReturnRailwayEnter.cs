...
{
    public partial class SubProductReturnRailwayEnter : Form
    {
        ...
        public SubProductReturnRailwayEnter()
        {
            InitializeComponent();
            Common.PlayWelcome();
            InitForm();
        ...
        private void InitForm()
        {
            ...
            this.textBoxPrompt.Text = Common.GetHelpStr("子公司成品铁运退货入厂");
            this.timer.Start();
        }

        private void toolStripButtonQuit_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("确实退出该界面吗?", "史丹利地磅系统-子公司成品铁运退货入厂", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                this.Close();
            }
        }
        ...
        {
            if (MessageBox.Show("确实要保存吗?", "史丹利地磅系统-子公司成品退货进厂(铁运退货)", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
        ...
        {
            if (MessageBox.Show("确实要修改吗?", "史丹利地磅系统-子公司成品退货进厂(铁运退货)", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                if (father[0].ToString() == "")
                {
