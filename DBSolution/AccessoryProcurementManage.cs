...
{
    public partial class AccessoryProcurementManage : Form
    {
        string whereCondition = string.Empty;

        public AccessoryProcurementManage()
        {
            InitializeComponent();
            ...
            if (columnIndex == 1)
            {
                   ...
                   MessageBoxIcon.Question) == DialogResult.OK)
                {
                    try
                    {
                        ...
                        Sdl_AccessoryProcurementTitle model = Sdl_AccessoryProcurementTitleAdapter.GetSdl_AccessoryProcurementTitle(truckNum, ebeln, timeFlag);
                        Sdl_AccessoryProcurementTitleAdapter.DeleteSdl_AccessoryProcurementTitle(timeFlag, ebeln);

                        CompareModelHelper.SdlDB_Modules module = CompareModelHelper.SdlDB_Modules.AccessoryProcurementTitle;
                        CompareModelHelper.CompareModel(model, new Sdl_AccessoryProcurementTitle(), CompareModelHelper.EditType.Delete, Common.GetEnumDescription(module));

                        DataTable dt = Sdl_AccessoryProcurementDetailAdapter.GetSdl_AccessoryProcurementDetailSet("where timeflag = '" + model.TIMEFLAG + "' and ebeln = '" + ebeln + "'").Tables[0];
                        for (int i = 0; i < dt.Rows.Count; i++)
                        {
                            Sdl_AccessoryProcurementDetail fpp = Sdl_AccessoryProcurementDetailAdapter.GetSdl_AccessoryProcurementDetail(timeFlag, dt.Rows[i]["ebeln"].ToString(), dt.Rows[i]["ebelp"].ToString(), dt.Rows[i]["matnr"].ToString());
                            Sdl_AccessoryProcurementDetailAdapter.DeleteSdl_AccessoryProcurementDetail(timeFlag, dt.Rows[i]["ebeln"].ToString(), dt.Rows[i]["ebelp"].ToString(), dt.Rows[i]["matnr"].ToString());
                            module = CompareModelHelper.SdlDB_Modules.AccessoryProcurementDetail;
                            CompareModelHelper.CompareModel(fpp, new Sdl_AccessoryProcurementDetail(), CompareModelHelper.EditType.Delete, Common.GetEnumDescription(module));
                        }
        ...
        private void EditDetail(string truckNum, string ebeln, string timeFlag)
        {
            ...
            AccessoryProcurementEdit edit = new AccessoryProcurementEdit();
