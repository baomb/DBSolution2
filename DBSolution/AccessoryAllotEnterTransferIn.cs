...
{
    public partial class AccessoryAllotEnterTransferIn : Form
    {
        ...
        public AccessoryAllotEnterTransferIn()
        {
            InitializeComponent();
            InitForm();
            if (!readPort)
        ...
        private void InitForm()
        {
            ...
            this.textBoxPrompt.Text = Common.GetHelpStr("配件调拨(调入)入厂");
            this.timer.Start();
        }

        private void toolStripButtonQuit_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("确实要退出该界面吗?", "史丹利地磅系统-配件调拨(调入)入厂", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                this.Close();
            }
        }
        ...
        {
            if (MessageBox.Show("确实要保存吗?", "史丹利地磅系统-配件调拨(调入)入厂", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                ...
                DataTable dtenter = Sdl_AccessoryAllotInTitleAdapter.GetSdl_AccessoryAllotInTitleSetByFeild(new string[] { "ENTERTIME" }, " where trucknum='" + textBoxCar.Text + "' and HSFLAG = 'H' ").Tables[0];
                ...
                try
                {
                    for (int i = 0; i < dataGridViewDetails.Rows.Count; i++)
                    {
                        ...
                        Sdl_AccessoryAllotInTitleAdapter.AddSdl_AccessoryAllotInTitle(model);
                    }

                    Sdl_AccessoryAllotOutTitle outTitle = Sdl_AccessoryAllotOutTitleAdapter.GetSdl_AccessoryAllotOutTitle(textBoxCar.Text, textBoxEbeln.Text, father[2].ToString());
                    outTitle.ALLOTFLAG = 1;
                    Sdl_AccessoryAllotOutTitleAdapter.UpdateSdl_AccessoryAllotOutTitle(outTitle);
        ...
        private void textBoxCar_DoubleClick(object sender, EventArgs e)
        {
            ...
            TruckSelect ts = new TruckSelect(father, "AccessoryAllotTransferOutAndIn");
        ...
        private void InitSelectDataBind()
        {
            try
            {
                ...
                {
                    DataTable dtH = Sdl_AccessoryAllotOutDetailAdapter.GetSdl_AccessoryAllotOutDetailMengeAndSfimg(" WHERE EBELN = '" + dtS.Rows[i]["EBELN"].ToString()
