...
{
    public partial class SubProductReturnMerchantEnter : Form
    {
        ...
        public SubProductReturnMerchantEnter()
        {
            InitializeComponent();
            Common.PlayWelcome();
            InitForm();
        ...
        private void InitForm()
        {
            ...
            this.textBoxPrompt.Text = Common.GetHelpStr("子公司成品经销商退货入厂");
            this.timer.Start();
        }
        ...
        {
            if (MessageBox.Show("确实退出该界面吗?", "史丹利地磅系统-子公司成品经销商退货入厂", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                this.Close();
            }
        }
        ...
        {
            if (MessageBox.Show("确实要保存吗?", "史丹利地磅系统-总公司成品退货入厂", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                ...
                {
                    DataTable dtenter = Sdl_ProductReturnMerchantAdapter.GetSdl_ProductReturnMerchantSetByFeild(new string[] { "ENTERTIME" }, " where trucknum='" + textBoxCar.Text + "' and HSFLAG = 'H' ").Tables[0];
                ...
                try
                {
                    for (int i = 0; i < dataGridViewSelect.Rows.Count; i++)
                    {
                        if (dataGridViewSelect.Rows[i].Cells[0].EditedFormattedValue.ToString().ToUpper() == "TRUE")
                        {
                            Sdl_ProductReturnMerchant fps = new Sdl_ProductReturnMerchant();
                            ...
                            Sdl_ProductReturnMerchantAdapter.AddSdl_ProductReturnMerchant(fps);
                        }
                    }
            ...
            if (MessageBox.Show("确实要修改吗?", "史丹利地磅系统-原材料退货入厂", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                ...
                try
                {
                    ...
                    try
                    {
                        ...
                        sql.AppendFormat(" delete from Sdl_ProductReturnMerchant where  trucknum='{0}' and timeFlag='{1}' ; ", textBoxCar.Text, father[1].ToString());
                        CommonOper.ExecuteSql(sql.ToString());
                        for (int i = 0; i < dataGridViewSelect.Rows.Count; i++)
                        {
                            if (dataGridViewSelect.Rows[i].Cells[0].EditedFormattedValue.ToString().ToUpper() == "TRUE")
                            {
                                Sdl_ProductReturnMerchant fpst = new Sdl_ProductReturnMerchant();
