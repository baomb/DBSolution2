...
{
    public partial class SubProductReturnRailwayExit : Form
    {
        ...
        public SubProductReturnRailwayExit()
        {
            InitializeComponent();
            InitForm();
            if (!readPort)
        ...
        private void InitForm()
        {
            ...
            this.textBoxPrompt.Text = Common.GetHelpStr("子公司成品铁运退货出厂");
            this.timer.Start();
        }

        private void toolStripButton4_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("确实退出该界面吗?", "史丹利地磅系统-子公司成品铁运退货出厂", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                this.Close();
            }
        }
        ...
        private bool ValidateControl(DataTable dt)
        {
            ...
            {
                if (MessageBox.Show("实退吨数与地磅吨数应该相等,确认退货过账吗？", "史丹利地磅系统-子公司成品退货出厂(铁运)", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
        ...
        {
            if (MessageBox.Show("确实要保存吗?", "史丹利地磅系统-子公司成品退货出厂(铁运)", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
        ...
        private void toolStripButtonCancel_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("确实要重车出厂吗?", "史丹利地磅系统-子公司成品退货出厂(铁运)", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                ...
                {
                    if (MessageBox.Show("重车出厂时，毛重与皮重数值应该相等，确认放行吗？", "史丹利地磅系统-子公司成品退货出厂(铁运)", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                    {
