...
{
    public partial class AllotTransferInManage : Form
    {
        string whereCondition = string.Empty;

        public AllotTransferInManage()
        {
            InitializeComponent();
            ...
            if (columnIndex == 1)
            {
                   ...
                   MessageBoxIcon.Question) == DialogResult.OK)
                {
                    try
                    {
                        ...
                        Sdl_AllotInTitle model = Sdl_AllotInTitleAdapter.GetSdl_AllotInTitle(truckNum, ebeln, timeFlag);
                        DataTable dt = Sdl_AllotInDetailAdapter.GetSdl_AllotInDetailSet("where timeflag = '" + model.TIMEFLAG + "' and ebeln = '" + ebeln + "'").Tables[0];

                        Sdl_AllotInTitleAdapter.DeleteSdl_AllotInTitle(timeFlag, ebeln, truckNum);

                        CompareModelHelper.SdlDB_Modules module = CompareModelHelper.SdlDB_Modules.AccessoryAllotInTitle;
                        CompareModelHelper.CompareModel(model, new Sdl_AllotInTitle(), CompareModelHelper.EditType.Delete, Common.GetEnumDescription(module));

                        for (int i = 0; i < dt.Rows.Count; i++)
                        {
                            Sdl_AllotInDetail fpp = Sdl_AllotInDetailAdapter.GetSdl_AllotInDetail(timeFlag, ebeln, dt.Rows[i]["ebelp"].ToString(), dt.Rows[i]["lgort"].ToString());
                            Sdl_AllotInDetailAdapter.DeleteSdl_AllotInDetail(timeFlag, ebeln, dt.Rows[i]["ebelp"].ToString(), dt.Rows[i]["lgort"].ToString());
                            module = CompareModelHelper.SdlDB_Modules.AccessoryAllotInDetail;
                            CompareModelHelper.CompareModel(fpp, new Sdl_AllotInDetail(), CompareModelHelper.EditType.Delete, Common.GetEnumDescription(module));
                        }
        ...
        private void EditDetail(string truckNum, string timeFlag)
        {
            ...
            AllotTransferInEdit edit = new AllotTransferInEdit();
