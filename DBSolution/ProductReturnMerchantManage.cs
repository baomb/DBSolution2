...
{
    public partial class ProductReturnMerchantManage : Form
    {
        string whereCondition = string.Empty;

        public ProductReturnMerchantManage()
        {
            InitializeComponent();
        ...
        private void SearchDataBind(int pageIndex, string whereCondition)
        {
            ...
            DataSet ds = Sdl_ProductReturnMerchantAdapter.GetSdl_ProductReturnMerchantPageData(pageIndex.ToString(), pager.PageSize, whereCondition);
            ...
            if (columnIndex == 1)
            {
                   ...
                   MessageBoxIcon.Question) == DialogResult.OK)
                {
                    try
                    {
                        ...
                        Sdl_ProductReturnMerchant model = Sdl_ProductReturnMerchantAdapter.GetSdl_ProductReturnMerchant(truckNum, vbeln, timeFlag);
                        ...
                        CompareModelHelper.CompareModel(model, new Sdl_ProductReturnMerchant(), CompareModelHelper.EditType.Delete, Common.GetEnumDescription(module));

                        DataTable dt = Sdl_ProductReturnMerchantDetailAdapter.GetSdl_ProductReturnMerchantDetailSet("where timeflag = '" + model.TIMEFLAG + "' and vbeln = '" + model.VBELN + "'").Tables[0];
                        for (int i = 0; i < dt.Rows.Count; i++)
                        {
                            Sdl_ProductReturnMerchantDetail fps = Sdl_ProductReturnMerchantDetailAdapter.GetSdl_ProductReturnMerchantDetail(dt.Rows[i]["timeflag"].ToString(), dt.Rows[i]["vbeln"].ToString(), dt.Rows[i]["posnr"].ToString(), dt.Rows[i]["lgort"].ToString());
                            Sdl_ProductReturnMerchantDetailAdapter.DeleteSdl_ProductReturnMerchantDetail(dt.Rows[i]["timeflag"].ToString(), dt.Rows[i]["vbeln"].ToString(), dt.Rows[i]["lgort"].ToString(), dt.Rows[i]["posnr"].ToString());

                            module = CompareModelHelper.SdlDB_Modules.ProductReturnMerchantDetail;
        ...
        private void EditDetail(string truckNum, string timeFlag)
        {
            ...
            ProductReturnMerchantEdit edit = new ProductReturnMerchantEdit();
