...
namespace DBSolution
{
    public partial class RawMaterialReturnExit : Form
    {
        ...
        private bool ValidateControl(DataTable dt)
        {
            ...
            if (Convert.ToDouble(textBoxDeductNum.Text) > balanceValue)
            {
                MessageBox.Show(this, "扣杂不在误差" + balanceValue + "吨允许范围内", "错误", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return false;
            }
             */
            //double Dvalue = Math.Round((double.Parse(textBoxNet.Text) + double.Parse(textBoxDeductNum.Text)), 3);
            ...
            //{
            //    MessageBox.Show(this, "实退数量与实际过磅数量不相符！", "错误", MessageBoxButtons.OK, MessageBoxIcon.Error);
            //    return false;
            //}
