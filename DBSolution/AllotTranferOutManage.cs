...
{
    public partial class AllotTransferOutManage : Form
    {
        string whereCondition = string.Empty;

        public AllotTransferOutManage()
        {
            InitializeComponent();
            ...
            if (columnIndex == 1)
            {
                   ...
                   MessageBoxIcon.Question) == DialogResult.OK)
                {
                    try
                    {
                        ...
                        Sdl_AllotTitle model = Sdl_AllotTitleAdapter.GetSdl_AllotTitle(truckNum, ebeln, timeFlag);
                        DataTable dt = Sdl_AllotDetailAdapter.GetSdl_AllotDetailSet("where timeflag = '" + model.TIMEFLAG + "' and ebeln = '" + ebeln + "'").Tables[0];
                        Sdl_AllotTitleAdapter.DeleteSdl_AllotTitle(timeFlag, ebeln, truckNum);

                        CompareModelHelper.SdlDB_Modules module = CompareModelHelper.SdlDB_Modules.AllotTitle;
                        CompareModelHelper.CompareModel(model, new Sdl_AllotTitle(), CompareModelHelper.EditType.Delete, Common.GetEnumDescription(module));

                        for (int i = 0; i < dt.Rows.Count; i++)
                        {
                            Sdl_AllotDetail fpp = Sdl_AllotDetailAdapter.GetSdl_AllotDetail(timeFlag, ebeln, dt.Rows[i]["ebelp"].ToString(), dt.Rows[i]["lgort"].ToString());
                            Sdl_AllotDetailAdapter.DeleteSdl_AllotDetail(timeFlag, ebeln, dt.Rows[i]["ebelp"].ToString(), dt.Rows[i]["lgort"].ToString());
                            module = CompareModelHelper.SdlDB_Modules.AllotDetail;
                            CompareModelHelper.CompareModel(fpp, new Sdl_AllotDetail(), CompareModelHelper.EditType.Delete, Common.GetEnumDescription(module));
                        }
        ...
        private void EditDetail(string truckNum, string timeFlag)
        {
            ...
            AllotTransferOutEdit edit = new AllotTransferOutEdit();
