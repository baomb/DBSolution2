...
{
    public partial class AccessoryAllotEnterTransferOut : Form
    {
        ...
        public AccessoryAllotEnterTransferOut()
        {
            InitializeComponent();
            InitForm();
            if (!readPort)
        ...
        private void InitForm()
        {
            ...
            this.textBoxPrompt.Text = Common.GetHelpStr("配件调拨(调出)入厂");
            this.timer.Start();
        }

        private void toolStripButtonQuit_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("确实要退出该界面吗?", "史丹利地磅系统-配件调拨(调出)入厂", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                this.Close();
            }
        }
        ...
        {
            if (MessageBox.Show("确实要保存吗?", "史丹利地磅系统-配件调拨(调出)入厂", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                ...
                {
                    MessageBox.Show(this, "没有该调拨单行信息，请重新调用", "错误", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }
                ...
                DataTable dtenter = Sdl_AccessoryAllotOutTitleAdapter.GetSdl_AccessoryAllotOutTitleSetByFeild(new string[] { "ENTERTIME" }, " where trucknum='" + textBoxCar.Text + "' and HSFLAG = 'H' ").Tables[0];
                ...
                try
                {
                    for (int i = 0; i < dataGridViewDetails.Rows.Count; i++)
                    {
                        ...
                        Sdl_AccessoryAllotOutTitleAdapter.AddSdl_AccessoryAllotOutTitle(model);
                    }
        ...
        {
            if (MessageBox.Show("确实要修改吗?", "史丹利地磅系统-配件调拨(调出)入厂", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                ...
                {
                    Sdl_AccessoryAllotOutTitle model = new Sdl_AccessoryAllotOutTitle();
                    model = Sdl_AccessoryAllotOutTitleAdapter.GetSdl_AccessoryAllotOutTitle(father[0].ToString(), father[1].ToString(), father[2].ToString());
                    if (model == null)
                    {
                    ...
                    model.ENTERWEIGHMAN = textBoxWeighMan.Text;
                    model.DBNUM = Sdl_SysSettingAdapter.LoadSdl_SysSetting().ID;
                    Sdl_AccessoryAllotOutTitleAdapter.UpdateSdl_AccessoryAllotOutTitleByTimeFlag(model);
        ...
        private void InitSelectDataBind()
        {
            try
            {
                ...
                {
                    double overNum = Sdl_AccessoryAllotOutDetailAdapter.GetSdl_AccessoryAllotOutDetailOverNum(" WHERE EBELN = '" + dtS.Rows[i]["EBELN"].ToString() + "' and EBELP='" + dtS.Rows[i]["EBELP"].ToString() + "' ");
        ...
        {
            Sdl_AccessoryAllotOutTitle title = Sdl_AccessoryAllotOutTitleAdapter.GetSdl_AccessoryAllotOutTitle(textBoxCar.Text, father[2].ToString());
            if (title != null)
            {
