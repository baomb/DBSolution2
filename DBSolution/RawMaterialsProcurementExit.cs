...
{
    public partial class RawMaterialsProcurementExit : Form
    {
        bool readPort = true;
        decimal net2 = 0;
        decimal discount = 0;
        ...
        public RawMaterialsProcurementExit()
        {
            InitializeComponent();
            InitForm();
            if (!readPort)
        ...
        private void textBoxCar_DoubleClick(object sender, EventArgs e)
        {
            ...
            lbFlag = string.Empty;
            TruckSelect ts = new TruckSelect(father, "RawMaterialsProcurement");
        ...
        private void InitDetailsDataBind(string where)
        {
            ...
            lt.Add("ZEPO", "EBELN,EBELP,MATNR,MAKTX,LIFNR,NAME1,MENGE,PTEXT,ANGNR");
            ...
            for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
            {
                try
                {
                    if (ds.Tables[0].Rows[i]["ANGNR"].ToString() == "1")
                    {
                        lbFlag = "2";
                    }
                    else
                    ...
                    if( (ebeln.Length - ds.Tables[0].Rows[i]["EBELN"].ToString().Length) != 0)
                    {
                        ds.Tables[0].Rows[i]["EBELN"] = ebeln;
                    }
                    Sdl_RawMaterialsProcurement rmp = Sdl_RawMaterialsProcurementAdapter.GetSdl_RawMaterialsProcurement(father[2].ToString(), ebeln, ds.Tables[0].Rows[i]["EBELP"].ToString());
                    if (rmp.LFIMG == 0)
                    {
            ...
            //cbst.DefaultCellStyle.NullValue = "标准";
        ...
        private void toolStripButtonSave_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    if (dt.Rows[i]["EBELN"].ToString().Length != ebeln.Length)
                    {
                        dt.Rows[i]["EBELN"] = ebeln;
                    }
            ...
            try
            {
                ...
                for (int i = 0; i < dataGridViewDetails.Rows.Count; i++)
                {
                    ...
                    rmp.STORAGETYPE = dataGridViewDetails.Rows[i].Cells["StorageType"].Value.ToString();
                ...
                {
                    rmpt.TRAYWEIGHT = Convert.ToDecimal(comboBoxStandardWeight.Text);
                }
                if (ValidateHelper.IsNumber(this.textBoxQuantity.Text))
                {
                    rmpt.TRAYQUANTITY = Convert.ToInt32(textBoxQuantity.Text);
                }
            ...
            try
            {
                ...
                for (int i = 0; i < dataGridViewDetails.Rows.Count; i++)
                {
                    if (dataGridViewDetails.Rows[i].Cells["EBELP"].Value.ToString() != temp)
                    {
                        ...
                        temp = dataGridViewDetails.Rows[i].Cells["EBELP"].Value.ToString();
                    }else if (dataGridViewDetails.Rows[i].Cells["EBELP"].Value.ToString() == temp && i > 0 && dataGridViewDetails.Rows[i].Cells["PWEIGHT"].Value.ToString() != dataGridViewDetails.Rows[i - 1].Cells["PWEIGHT"].Value.ToString())
                    {
                        total += (dataGridViewDetails.Rows[i].Cells["LFIMG"].Value == DBNull.Value) ? 0 : Convert.ToDecimal(dataGridViewDetails.Rows[i].Cells["LFIMG"].Value);
                        temp = dataGridViewDetails.Rows[i].Cells["EBELP"].Value.ToString();
                    }
