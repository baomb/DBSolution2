...
{
    public partial class FinishedProductsPresentationManage : Form
    {
        string whereCondition = string.Empty;

        public FinishedProductsPresentationManage()
        {
            InitializeComponent();
        ...
        private string GetWhereStr()
        {
            ...
            {
                condition += " and rsnum = '" + textRSNUM.Text + "'";
            }
        ...
        private void dataGridViewDetail_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            string truckNum, timeFlag, rsnum;
            if (columnIndex == 1)
            {
                   ...
                   MessageBoxIcon.Question) == DialogResult.OK)
                {
                    try
                    {
                        ...
                        rsnum = dataGridViewDetail.Rows[rowIndex].Cells["RSNUM"].Value as string;
                        Sdl_FinishedProductsPresentationTitle model = Sdl_FinishedProductsPresentationTitleAdapter.GetSdl_FinishedProductsPresentationTitle(truckNum, rsnum, timeFlag);

                        Sdl_FinishedProductsPresentationTitleAdapter.DeleteSdl_FinishedProductsPresentationTitle(timeFlag, rsnum, truckNum);

                        CompareModelHelper.SdlDB_Modules module = CompareModelHelper.SdlDB_Modules.FinishedProductsPresentationTitle;
                        CompareModelHelper.CompareModel(model, new Sdl_FinishedProductsPresentationTitle(), CompareModelHelper.EditType.Delete, Common.GetEnumDescription(module));

                        DataTable dt = Sdl_FinishedProductsPresentationAdapter.GetSdl_FinishedProductsPresentationDataSet("where timeflag = '" + model.TIMEFLAG + "' and rsnum = '" + rsnum + "'").Tables[0];
                        for (int i = 0; i < dt.Rows.Count; i++)
                        {
                            Sdl_FinishedProductsPresentation fpp = Sdl_FinishedProductsPresentationAdapter.GetSdl_FinishedProductsPresentation(timeFlag, rsnum, dt.Rows[i]["rspos"].ToString(), dt.Rows[i]["lgort"].ToString());
                            Sdl_FinishedProductsPresentationAdapter.DeleteSdl_FinishedProductsPresentation(timeFlag, rsnum, dt.Rows[i]["rspos"].ToString(), dt.Rows[i]["lgort"].ToString());
                            module = CompareModelHelper.SdlDB_Modules.FinishedProductsPresentation;
                            CompareModelHelper.CompareModel(fpp, new Sdl_FinishedProductsPresentation(), CompareModelHelper.EditType.Delete, Common.GetEnumDescription(module));
                        }
            ...
            else if (columnIndex == 0 && rowIndex != -1 && !dataGridViewDetail.Rows[rowIndex].IsNewRow)
            {
                ...
                if (truckNum != null)
                {
                    this.EditDetail(truckNum, rsnum, timeFlag);
                }
            }
        }

        private void EditDetail(string truckNum, string rsnum, string timeFlag)
        {
            this.Cursor = Cursors.WaitCursor;
            FinishedProductsPresentationEdit edit = new FinishedProductsPresentationEdit();
            edit.StartPosition = FormStartPosition.CenterParent;
            edit.ShowDialog(this, truckNum, rsnum, timeFlag);
