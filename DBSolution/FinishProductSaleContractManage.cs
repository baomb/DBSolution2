...
{
    public partial class FinishProductSaleContractManage : Form
    {
        string whereCondition = string.Empty;
        public FinishProductSaleContractManage()
        {
            InitializeComponent();
            ...
            if (columnIndex == 1)
            {
                   ...
                   MessageBoxIcon.Question) == DialogResult.OK)
                {
                    try
                    {
                        ...
                        Sdl_FinishedProductsSaleTitle model = Sdl_FinishedProductsSaleTitleAdapter.GetSdl_FinishedProductsSaleTitle(truckNum, vbeln, timeFlag);

                        Sdl_FinishedProductsSaleTitleAdapter.DeleteSdl_FinishedProductsSaleTitle(model.TRUCKNUM, model.TIMEFLAG, model.VBELN);

                        CompareModelHelper.SdlDB_Modules module = CompareModelHelper.SdlDB_Modules.FinishedProductsSaleTitle;
                        CompareModelHelper.CompareModel(model, new Sdl_FinishedProductsSaleTitle(), CompareModelHelper.EditType.Delete, Common.GetEnumDescription(module));
                        DataTable dt = Sdl_FinishedProductsSaleAdapter.GetSdl_FinishedProductsSaleSet("where timeflag = '" + model.TIMEFLAG + "' and vbeln = '" + model.VBELN + "'").Tables[0];
                        for (int i = 0; i < dt.Rows.Count; i++)
                        {
                            Sdl_FinishedProductsSale fps = Sdl_FinishedProductsSaleAdapter.GetSdl_FinishedProductsSale(dt.Rows[i]["timeflag"].ToString(), dt.Rows[i]["vbeln"].ToString(), dt.Rows[i]["posnr"].ToString(), dt.Rows[i]["lgort"].ToString());
                            Sdl_FinishedProductsSaleAdapter.DeleteSdl_FinishedProductsSale(dt.Rows[i]["timeflag"].ToString(), dt.Rows[i]["vbeln"].ToString(), dt.Rows[i]["lgort"].ToString(), dt.Rows[i]["posnr"].ToString());

                            module = CompareModelHelper.SdlDB_Modules.FinishedProductsSale;
        ...
        private void EditDetail(string truckNum, string timeFlag)
        {
            ...
            FinishedProductsSaleEdit edit = new FinishedProductsSaleEdit();
