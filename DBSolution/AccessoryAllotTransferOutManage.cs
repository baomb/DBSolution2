...
{
    public partial class AccessoryAllotTransferOutManage : Form
    {
        string whereCondition = string.Empty;

        public AccessoryAllotTransferOutManage()
        {
            InitializeComponent();
        ...
        private void SearchDataBind(int pageIndex, string whereCondition)
        {
            ...
            DataSet ds = Sdl_AccessoryAllotOutTitleAdapter.GetSdl_AccessoryAllotOutTitlePageData(pageIndex.ToString(), pager.PageSize, whereCondition);
            ...
            if (columnIndex == 1)
            {
                   ...
                   MessageBoxIcon.Question) == DialogResult.OK)
                {
                    try
                    {
                        ...
                        Sdl_AccessoryAllotOutTitle model = Sdl_AccessoryAllotOutTitleAdapter.GetSdl_AccessoryAllotOutTitle(truckNum, ebeln, timeFlag);

                        Sdl_AccessoryAllotOutTitleAdapter.DeleteSdl_AccessoryAllotOutTitle(timeFlag, ebeln, truckNum);

                        CompareModelHelper.SdlDB_Modules module = CompareModelHelper.SdlDB_Modules.AccessoryAllotOutTitle;
                        CompareModelHelper.CompareModel(model, new Sdl_AccessoryAllotOutTitle(), CompareModelHelper.EditType.Delete, Common.GetEnumDescription(module));

                        DataTable dt = Sdl_AccessoryAllotOutDetailAdapter.GetSdl_AccessoryAllotOutDetailSet("where timeflag = '" + model.TIMEFLAG + "' and werks = '" + model.WERKS + "' and ebeln = '" + ebeln + "'").Tables[0];
                        for (int i = 0; i < dt.Rows.Count; i++)
                        {
                            Sdl_AccessoryAllotOutDetail fpp = Sdl_AccessoryAllotOutDetailAdapter.GetSdl_AccessoryAllotOutDetail(timeFlag, ebeln, dt.Rows[i]["ebelp"].ToString(), dt.Rows[i]["lgort"].ToString());
                            Sdl_AccessoryAllotOutDetailAdapter.DeleteSdl_AccessoryAllotOutDetail(timeFlag, ebeln, dt.Rows[i]["ebelp"].ToString(), dt.Rows[i]["lgort"].ToString());
                            module = CompareModelHelper.SdlDB_Modules.AccessoryAllotOutDetail;
                            CompareModelHelper.CompareModel(fpp, new Sdl_AccessoryAllotOutDetail(), CompareModelHelper.EditType.Delete, Common.GetEnumDescription(module));
                        }
        ...
        private void EditDetail(string truckNum, string timeFlag)
        {
            ...
            AccessoryAllotTransferOutEdit edit = new AccessoryAllotTransferOutEdit();
