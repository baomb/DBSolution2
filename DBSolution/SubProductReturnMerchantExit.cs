...
{
    public partial class SubProductReturnMerchantExit : Form
    {
        ...
        public SubProductReturnMerchantExit()
        {
            InitializeComponent();
            InitForm();
            if (!readPort)
        ...
        private void InitForm()
        {
            ...
            this.textBoxPrompt.Text = Common.GetHelpStr("子公司成品经销商退货出厂");
            this.timer.Start();
        }

        private void toolStripButton4_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("确实退出该界面吗?", "史丹利地磅系统-子公司成品经销商退货出厂", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                this.Close();
            }
        }
        ...
        private bool ValidateControl(DataTable dt)
        {
            ...
            {
                if (MessageBox.Show("实退吨数与实际过磅数量应该相等，确认过账吗?", "子公司成品退货出厂(经销商)", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    return true;
                }
                return false;
        ...
        {
            if (MessageBox.Show("确实要保存吗?", "史丹利地磅系统-子公司成品退货出厂(经销商)", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                ...
                try
                {
                    ...
                    try
                    {
                        dtGZ = SAPHelper.InvokSAPFunTable("Z_SDL_FERT_MUTUAL_ZGS_RE_GZ", ltPara, ltParaTb, lt, ref lr);
                    }
                    catch
                    {
                    ...
                    try
                    {
                        ...
                        for (int i = 0; i < dataGridViewSelect.Rows.Count; i++)
                        {
                            if (dataGridViewSelect.Rows[i].Cells[0].EditedFormattedValue.ToString().ToUpper() == "TRUE")
                            {
                                ...
                                model.KUIJIAN = 0;// double.Parse(textBoxKuijian.Text);
                                model.TYPEID = 0;
                                if (model.TRUCKNUM == "")
                                {
        ...
        {
            if (MessageBox.Show("确实要重车出厂吗?", "史丹利地磅系统-子公司成品退货出厂(经销商)", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                if (toolStripButton.Text != "解锁")
                {
                    MessageBox.Show(this, "请先锁定地磅读数", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
                ...
                {
                    if (MessageBox.Show("重车出厂时，毛重与皮重数值应该相等，确认放行吗？", "史丹利地磅系统-子公司成品退货出厂(经销商)", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                    {
