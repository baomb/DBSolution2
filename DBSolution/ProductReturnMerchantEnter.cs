...
{
    public partial class ProductReturnMerchantEnter : Form
    {
        ...
        public ProductReturnMerchantEnter()
        {
            InitializeComponent();
            Common.PlayWelcome();
            InitForm();
        ...
        private void InitForm()
        {
            ...
            this.textBoxPrompt.Text = Common.GetHelpStr("总公司成品经销商退货入厂");
            this.timer.Start();
        }

        private void toolStripButtonQuit_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("确实退出该界面吗?", "史丹利地磅系统-总公司成品经销商退货入厂", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                this.Close();
            }
        }
        ...
        {
            if (MessageBox.Show("确实要保存吗?", "史丹利地磅系统-总公司成品退货入厂(经销商)", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
