...
{
    public partial class FinishProductSaleContractExit : Form
    {
        ...
        public FinishProductSaleContractExit()
        {
            InitializeComponent();
            InitForm();
            if (!readPort)
        ...
        private void InitForm()
        {
            ...
            this.textBoxPrompt.Text = Common.GetHelpStr("子公司成品销售出厂");
            //判断是否启用托盘
        ...
        private void textBoxCar_DoubleClick(object sender, EventArgs e)
        {
            ...
            TruckSelect ts = new TruckSelect(father, "ContractSale");
            ts.ShowDialog();
        ...
        private void InitDetailBind(string where)
        {
            ...
            DataTable dtCombox = Sdl_WarehouseAdapter.GetSdl_WarehouseSet(" where ( lgort like '1%'  or lgort like '3%55' ) and werks='" + textBoxFactory.Text + "' ").Tables[0];
        ...
        private bool ValidateControl(DataTable dt)
        {
            ...
            {
                if (MessageBox.Show("原发吨数与实际过磅数量在误差件数" + lfZfimgSum + "乘以包误差" + ProBalance1 * 1000 + "KG允许的范围内不相等,确认过账吗？", "史丹利地磅系统-子公司成品销售出厂", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    return true;
                }
                return false;
        ...
        {
            String dateTime = textBoxExitTime.Text.ToString().Substring(0, 10).Replace("-", "");
            if (MessageBox.Show("确实要保存吗?", "史丹利地磅系统-子公司成品销售出厂", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                ...
                {
                    //使用SAP通信
                    DataSet dtInfo = SAPHelper.InvokSAPFunTable("Z_SDL_FERT_MUTUAL_ZGS_GZ_CHECK", ltPara, ltParaTb, lt, ref lr);
                    ...
                    try
                    {
                        ltPara.Add("IDATUM", dateTime);
                        dtGZ = SAPHelper.InvokSAPFunTable("Z_SDL_FERT_MUTUAL_ZGS_GZ_CO", ltPara, ltParaTb, lt, ref lr);
                    }
                    catch
                    {
                    ...
                    try
                    {
                        ...
                        //使用SAP通信
                        //SAPHelper.InvokSAPFunTableVB("Z_SDL_ZGS_SEND", ld1, ldVBELN, ld2, ref ld1);
        ...
        {
            if (MessageBox.Show("确实要退出该界面吗?", "史丹利地磅系统-子公司成品销售出厂", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                this.Close();
            }
        }
        ...
        {
            if (MessageBox.Show("确认空车出厂操作吗?", "史丹利地磅系统-子公司成品销售出厂", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                if (toolStripButton.Text != "解锁")
                {
                    MessageBox.Show(this, "请先锁定地磅读数", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
                ...
                //if (netValue >= -balanceValue && netValue<=balanceValue)
                ...
                try
                {
                    ...
                    {
                        for (int i = 0; i < dtVBELN.Rows.Count; i++)
                        {
                            dr = dtNewWW.NewRow();
                            dr["VBELN"] = dtVBELN.Rows[i]["VBELN"].ToString();
                            dr["MZ"] = textBoxGross.Text;
                            dr["NUMBR"] = textBoxCar.Text;
                            dr["FLAG"] = "1";
                            dr["EFLAG"] = "1";
                            dr["NRDAT"] = textBoxExitTime.Text.ToString().Substring(0, 10).Replace("-", "");
                ...
                else
                {
                    ...
                    modelEnter.GROSS = double.Parse(textBoxGross.Text);
                    modelEnter.EXITWEIGHMAN = textBoxWeighMan.Text;
                    modelEnter.EXITTIME = DateTime.Parse(textBoxExitTime.Text);
                    //Sdl_FinishedProductsSaleTitleAdapter.UpdateSdl_FinishedProductsSaleTitle(modelEnter);
                    Sdl_FinishedProductsSaleTitleAdapter.UpdateSdl_FinishedProductsSaleTitle_S(modelEnter);
                }
