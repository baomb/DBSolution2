...
namespace SdlDB.Data.SqlServer
{
    public class PageSqlHelper
    {
        ...
        /// <returns></returns>
        /// bmb测试结果，输入：Common.GetCanPageSql("99",88,"123##PAGE_BEGIN##456##PAGE_WITH_END##789##PAGE_END##012")
        ...
        public static string GetCanPageSql(string pageNum, int PageSize, string aSQL)
        {
            string sql_begin = "";
            string sql_with = "";
            string sql = aSQL;
            ...
            string strComm = " " + sql_with + " Select top " + Convert.ToInt32(pageNum) * PageSize + " identity(int,1,1) as r@o#w$i@d ,* into #tempTable from (" + sql + ")as a    ";
            strComm += "   select * from (Select  * from #tempTable where r@o#w$i@d between " + ((Convert.ToInt32(pageNum) - 1) * PageSize + 1) + " and " + Convert.ToInt32(pageNum) * PageSize + ") as b order by r@o#w$i@d  ; ";
